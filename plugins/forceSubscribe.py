        ...
        for channel in channel_s:
          ...
          if len(n_subscription_s) > 0:
            client.answer_callback_query(
              cb.id,
              text="❗ klik subs 'channel(s)' kemudian tekan 'UnMute Me' buat unmute.",
              show_alert=True
            )
          else:
      ...
      if len(n_subscription_s) > 0:
        ...
        try:
          sent_message = message.reply_text(
              "{}, kamu **belom subs** channel: {mmo}. klik subs di [join](https://t.me/{}) kemudian **klik unmute** buat unmute.".format(message.from_user.mention, channel, channel),
...
def get_valid_channels(message: Message):
  ...
  for input_str in channel_pors:
    ...
    try:
      ...
      return False, f"❗ **ch**\n__bkn ch [channel](https://t.me/{input_str}). Add me as a admin in order to enable ForceSubscribe.__"
